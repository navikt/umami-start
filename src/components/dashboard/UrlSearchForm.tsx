...
function UrlSearchForm({ children }: UrlSearchFormProps) {
  ...
  const fetchWebsites = async (): Promise<Website[]> => {
    ...
    try {
      ...
      const filteredItems = prodWebsites.filter(
        (item: Website) => item.domain !== "nav.no",
      );

      // Deduplicate by domain
      const uniqueWebsites = filteredItems.filter(
        (website: Website, index: number, self: Website[]) =>
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (matchedWebsite) {
        ...
        navigate(
          `/trafikkanalyse?websiteId=${matchedWebsite.id}&domain=${matchedWebsite.domain}&urlPath=${encodeURIComponent(decodeURIComponent(urlObj.pathname))}`,
        );
      } else {
    ...
    <div>
      <form role="search" onSubmit={handleSubmit}>
        <div>
          ...
          {alertVisible && (
            <Alert style={{ marginTop: "20px" }} variant="warning">
              Denne siden har ikke fått støtte for Umami enda. Fortvil ikke —
              kontakt Team ResearchOps for å få lagt den til :)
            </Alert>
          )}
        </div>
        {children}
        <ReadMore
          ...
          header="Hvilke nettsider / apper støttes?"
          onOpenChange={handleReadMoreToggle}
        >
          {isLoading ? (
            <List as="ul">
              {[...Array(3)].map((_, index) => (
            ...
            <List as="ul">
              {filteredData &&
                filteredData.map((item) => (
                  <List.Item key={item.id}>{item.domain}</List.Item>
                ))}
            </List>
          )}
          <BodyShort className="mt-4 mb-2">
            Savner du en nettside eller app?{" "}
            <Link href="/komigang">Følg kom-i-gang-guiden!</Link>
          </BodyShort>
        </ReadMore>
      </form>
    </div>
